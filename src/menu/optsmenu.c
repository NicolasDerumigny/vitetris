...
static int op_inputsetup_select(int x, int y) {
  const char *items[3] = {"single player", "  player1", "  player2"};
...
static void tetrom_colors_menu(int x, int y) {
  ...
  menuhandler handlers[7] = {op_tcolor,    op_tcolor,     op_tcolor,
                             op_tcolor,    op_tcolor_gt4, op_tcolor_gt4,
                             op_tcolor_gt4};
  int i = 0;
  if (y + 6 >= term_height)
    y -= 7;
  ...
  while (1) {
    setcurs(x, y + i % 4);
    refreshwin(-1);
    switch (handle_menuitem_2cols(menu, 7, &i, 4, x, y + i % 4, handlers,
