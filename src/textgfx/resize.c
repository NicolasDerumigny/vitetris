...
static void resizehandler(int sig) { textgfx_flags |= TERM_RESIZED; }

void enable_term_resizing() { signal(SIGWINCH, resizehandler); }
...
static void entergame_redraw() {
  ...
  if (game->mode & MODE_NETWORK &&
      CONNECTED != (sock_flags & (CONNECTED | WAIT_PL2INGAME)))
...
#if !CURSES || KEY_RESIZE
  int w, h;
  if (!(textgfx_flags & TERM_RESIZED) || !in_menu && game &&
