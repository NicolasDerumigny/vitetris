...
static void _set_allegro_icon() { allegro_icon = icon_xpm; }
#endif
...
static void close_btn() { close_button_pressed = 1; }
...
BITMAP *set_screen(int fullscreen) {
  const unsigned char vgargb[16][3] = {0,   0,   0,   /* 0 black */
                                       170, 0,   0,   /* 1 red */
                                       0,   170, 0,   /* 2 green */
                                       170, 85,  0,   /* 3 yellow */
                                       0,   0,   170, /* 4 blue */
                                       ...
                                       85,  255, 255, 85,  85,  85,  255, 255,
                                       85,  255, 85,  255, 255, 255, 255, 255};
...
void toggle_fullscreen() {
  ...
  if (bmp->w == virt_screen->w)
    ...
    if (bmp->w > virt_screen->w) {
      int h = 400;
      if (in_menu) {
        blit(virt_screen, screen, 0, 384, 0, 464, 512, 16);
        h = 384;
      }
...
int is_outside_screen(int x, int y) {
  return x + win_x + margin_x >= term_width || y + win_y >= 25;
}
...
int default_bgdot() { return BULLET; }
