...
#endif
            "name NAME	Identify yourself as NAME when connecting to other "
            "players");
#endif
...
static void proc_cmdline_opt(const char *name, char *arg) {
  const struct cmdopt opts[] = {{"[wh]|width|height", cmd_wh},
                                {"[?h]|help", cmd_help},
#ifdef ALLEGRO
  ...
  for (i = 0; i < n; i++) {
    ...
    if (q = strchr(p, '=')) {
      *q = '\0';
      ...
      c = args[i + 1][0];
      if (c != '-' &&
          (p > args[i] || !islower(c) || namematches(p, "[?h]|help|connect")))
        q = args[i + 1];
#ifdef SOCKET
