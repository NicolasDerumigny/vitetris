...
static char *get_wonlost_stats_key(const char *me, const char *opp,
                                   char **key) {
  ...
  for (; o; o = o->next) {
    ...
    if (m && !strncmp(s, me, m) && s[m] == ' ' && p > s + m)
      s += m + 1;
    if (p && isdigit(p[1]) && n == p - s && !strncmp(s, opp, n) &&
        strlen(p) <= 8) {
      if (key)
        *key = opt_key(o);
      return p + 1;
...
static void setopt_wonlost(const char *key, const char *me, const char *opp,
                           const char *stats) {
  union val val;
  char buf[44];
