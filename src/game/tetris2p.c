...
static void gen_tetrom_seq(int i, int n) {
  n += i;
  if (n > tetrom_seq_len) {
    tetrom_seq = !tetrom_seq ? malloc(n) : realloc(tetrom_seq, n);
    if (tetrom_seq_len == -1)
...
static int draw_lineclear_step(struct player *plr, struct tetris2p *tet,
                               int first) {
...
static struct player *play_round() {
  ...
  if (!(game->mode & MODE_BTYPE) || game->player[i].lines)
    i = !i;
  if (gameover == 3 &&
      (!(game->mode & MODE_BTYPE) || !player1.lines == !player2.lines))
    i = 2;
  ...
  while (plr = play_round()) {
    ...
    if (plr && plr->score == 3) {
      ...
      if (NETPLAY && opponent_name[0])
        upd_wonlost_stats(my_name, opponent_name, player1.score == 3);
#endif
      ;
    } else
      plr = NULL;
