...
int randnum(int n) { return (int)(n * (rand() / (RAND_MAX + 1.0))); }
...
void lockpiece(struct player *plr) {
  ...
  do {
    ...
    while (!(p->blocks & 1 << x))
      x++;
    do
      plr->board[y] |= (uint_least32_t)p->color << 3 * (p->x + x);
    while (p->blocks & 1 << ++x);
...
int movedown(struct player *plr, int drop) {
  ...
  if (!drop && p->y >= 0) {
    if (plr->mvright_tm &&
        LOCK_DELAY - plr->falltime >= gettm(plr->mvright_tm) - plr->mvright_tm)
      moveright(plr);
  ...
  do {
    if (bl & 15) {
      ...
      while (!(bl & 1 << x))
        x++;
      do
        if (y + i >= p->y && p->blocks & 1 << 4 * (i - p->y + y) + x)
          bl ^= 1 << x;
      while (bl & 1 << ++x);
...
int pausegame() {
  ...
  do
    key = getkeypress_block(SINGLE_PL);
  while (gettm(t) - t < 500);
