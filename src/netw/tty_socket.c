...
static void getaddr(const char *tty, int server) {
  ...
  if (sock_addr)
    free(sock_addr);
  addr = malloc(offsetof(struct sockaddr_un, sun_path) + strlen(fname) + 1);
...
char *mksocket_local(const char *tty, int server) {
  ...
  if (!msg) {
    getaddr(tty, server);
    size = offsetof(struct sockaddr_un, sun_path) + strlen(SUN_PATH(sock_addr));
    if (server) {
      unlink(SUN_PATH(sock_addr));
      msg = bind_listen(size);
      chmod(SUN_PATH(sock_addr),
