...
int kb_readkey(unsigned char *dest) {
  const short funckeys[7] = {KEY_LEFT,  KEY_RIGHT,     KEY_UP, KEY_DOWN,
                             KEY_ENTER, KEY_BACKSPACE, KEY_DC};
  const unsigned char keys[7] = {MVLEFT,   MVRIGHT,   MVUP, MVDOWN,
...
int kb_toascii(const unsigned char *key) {
  short numpad[10] = {
#ifdef PDCURSES
      PAD0,   KEY_C1, KEY_C2, KEY_C3, KEY_B1, KEY_B2,
      KEY_B3, KEY_A1, KEY_A2, KEY_A3
#else
      0, KEY_C1, 0, KEY_C3, 0, KEY_B2,
      0, KEY_A1, 0, KEY_A3
#endif
  };
...
int inpselect_dev(int tm) { return 0; }
