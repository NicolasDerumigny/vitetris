...
static int readshiftkey() {
  const shift_keys[6] = {KEY_LSHIFT,   KEY_RSHIFT, KEY_LCONTROL,
                         KEY_RCONTROL, KEY_ALT,    KEY_ALTGR};
  int scan;
  int i;
  for (i = 0; i < 6; i++) {
    scan = shift_keys[i];
...
void js_pressbtn(int i, int b) { joysticks[i].btns_tm[b - '0'] = gettm(0); }

void js_releasebtn(int i, int b) { joysticks[i].btns_tm[b - '0'] = -1; }
